...
export default function TextInput ({ title, initialValue, onChange }: Props) {
  const [value, setValue] = useState<FilterField<string>>(initialValue || {type: "none", value: ""});
  ...
  return <div className="p-2 rounded-xl border flex flex-row gap-2 justify-between">
    ...
    <div className="flex flex-col gap-2 max-w-[50%]">
      ...
      <input type="text" className="rounded-xl p-2 border" value={value.value} onInput={valueChange} />
    </div>
  </div>
}